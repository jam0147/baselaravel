	...
	app = function($form, op){
		...
		$this.guardar = function(){
			...
			var $opciones = {
				...
				'success': function(r){
					aviso(r);
					if ($this.op.limpiaDespuesGuardar)
						$this._limpiar();
	...
	$.ajax({
	    ...
	    processData: false,
	 	...
	 	beforeSend: function(){ 
	 		...
	 		$("#mensaje_validacion").text('Enviando pago');
	 		$(".enviar").prop('disabled', true);
	 	},
	 	success : function(data,status,xhr) {
	 		aviso(data.msj);
	 		$("#mensaje_validacion").text(data.msj);
	 		...
	 		if(data.s == "s"){
	 			...
	 			$("#archivo_1").val('');
	 			$("#archivo_2").val('');
	 			setTimeout($('#pagar-listo').modal('hide'), 100000);
	 			tabla.fnDraw();
	 		}else{
...
function eliminarOrden($id, $option, nRow, session){
	$.ajax({
		url : $url +'eliminar',
		data: {
			'id' : $id,
		...
		type : 'POST',
	 	dataType : 'json',
	 	beforeSend: function(){ 
	 		$(".cancelar", nRow).prop('disabled', true);
	 	},
	 	success : function(data,status,xhr) {
	 		aviso(data);
	 		$(".cancelar", nRow).prop('disabled', false);
	 		if(data.s == "s"){
	 			tabla.fnDraw();
	 		}
	 	},
		...
		type : 'POST',
	 	...
	 	success : function(r,status,xhr) {
	 		if(r.s == "s"){
	 			...
	 			var $modal = $("#modal-detalle");
	 			$("#codigo_orden", $modal).text(r.id);
	 			$("#wallet", $modal).text(r.wallet);
	 			$("#bsf", $modal).text(r.monto_total_bs);
	 			$("#cantidad_onx", $modal).text(r.cant_onx);
	 			$("#observaciones", $modal).text(r.observaciones);
	 			$("#fecha", $modal).text(r.fecha);
	 		}
	 	},
	 	error : function(xhr, status) {
